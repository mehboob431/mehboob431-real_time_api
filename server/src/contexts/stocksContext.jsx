...
const StockProvider = ({ children }) => {
  ...
  const fetchAllStocks = async () => {
    setLoading(true);
    try {
      const response = await axios.get(globalConstantUtil.baseUrl + "/stock");
  ...
  const addStock = async (newStock) => {
    try {
      const response = await axios.post(
        globalConstantUtil.baseUrl + "/stock",
        newStock
      );
  ...
  const updateStock = async (id, updatedStock) => {
    try {
      const response = await axios.put(
        globalConstantUtil.baseUrl + `/stock/${id}`,
        updatedStock
      );
  ...
  const deleteStock = async (id) => {
    try {
      await axios.delete(globalConstantUtil.baseUrl + `/stock/${id}`);
